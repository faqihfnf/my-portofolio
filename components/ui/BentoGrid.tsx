...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("faqih.fnf@gmail.com");
    setCopied(true);
  };
  return (
    <div
      ...
      style={{
        background: "rgb(3,8,39)",
        backgroundColor: "linear-gradient(90deg, rgba(3,0,39,1) 0%, rgba(4,0,57,1) 17%, rgba(46,41,133,1) 57%)",
      }}
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div className={`absolute right-0 -bottom-5 ${id === 5 && "w-full opacity 80"}`}>{spareImg && <img src={spareImg} alt={spareImg} className="object-cover object-center w-full h-full" />}</div>

        {id === 6 && (
          <BackgroundGradientAnimation>{/* <div className="absolute z-50 flex items-center justify-center text-white font-bold px-4 pointer-events-none text-3xl text-center md:text-4xl lg:text-7xl"></div> */}</BackgroundGradientAnimation>
        )}
        <div className={cn(titleClassName, "group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10")}>
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-4">
                {["HTML", "CSS", "JavaScript"].map((item) => (
                  <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-black-200">
                    {item}
                  </span>
                ))}
          ...
          {id === 6 && (
            <div className="mt-5 relative">
              ...
              <CopyButton title={copied ? "Email is Copied" : "Copy My Email"} icon={<IoCopy />} position="right" otherClasses="bg-black-200" handleClick={handleCopy} />
            </div>
          )}
        </div>
      </div>
